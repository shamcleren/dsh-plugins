...
describe('Codex session UI', () => {
  it('adds a Codex header without an activity dock', () => {
    ...
    const events = vi.fn(() => () => {})
    const views = vi.fn(() => () => {})
    apply({ effect: callback => { callback() }, locale: { register: vi.fn(), bind: () => t }, connection: { rpc: { call: vi.fn() } }, uiConversation: { events: { register: events }, views: { register: views } }, slots: { inject: slots, register } })
    ...
    const header = register.mock.calls[0]![1]
    expect(header({ sessionId: 'first' }).props.sessionId).toBe('first')
    expect(header({ sessionId: 'second' }).props.sessionId).toBe('second')
