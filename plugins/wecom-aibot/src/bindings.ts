...
function parseDocument(text: string): BindingDocument {
  ...
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new Error('wecom-aibot bindings document must be an object')
  }
  ...
  if (document.version !== 1 || typeof document.bindings !== 'object'
    ...
    throw new Error('wecom-aibot bindings document has an unsupported format')
  }
  const bindings: Record<string, string> = {}
  for (const [key, sessionId] of Object.entries(document.bindings)) {
    if (!/^[a-f0-9]{64}$/u.test(key) || typeof sessionId !== 'string' || !sessionId.startsWith('session-wecom-')) {
      throw new Error('wecom-aibot bindings document contains an invalid binding')
    }
