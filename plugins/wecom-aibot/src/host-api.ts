...
export function createHostApi(
  ...
  /**
   * Event forwarding has to open before the prompt submission resolves, so it is
   * a wider test than `routesSession`. Questions keep the narrower test: they are
   * only answerable once a turn is live, and offering to answer one without an
   * active turn would strand the asker.
   */
  observesSession: (sessionId: string) => boolean = routesSession,
): HostApiProxy {
  ...
  ctx.on('session/event', (session, event) => { if (observesSession(session.id)) emit({ type: 'session/event', sessionId: session.id, event }) })
